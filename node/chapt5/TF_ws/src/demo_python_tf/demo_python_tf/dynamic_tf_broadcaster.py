...
class DynamicTFBroadcast(Node):      # 继承 Node类
    ...
    def publish_dynamic_tf(self):
        ...
        # 2. 给 该 消息接口的内容 赋值
        transform.header.frame_id = 'camera_link'                     # 父类 坐标系 的 id
        transform.header.stamp = self.get_clock().now().to_msg()    # 获取 时间辍（常用）
        transform.child_frame_id = 'bottle_link'                    # 子类 坐标系 的 id
        # 平移坐标系 设置
        ...
        # 3. 动态坐标系 发布 出去：
        self.dynamic_broadcaster_.sendTransform(transform)           # 使用 dynamicTransformBroadcaster
